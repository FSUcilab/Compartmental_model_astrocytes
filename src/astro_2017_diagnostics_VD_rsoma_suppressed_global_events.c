...
int next_time = 0;   // Global: helps identify where global spikes occur
int times_sz = 0;     // number of global spikes + 1 (last element is a time larger than will be reached by the simulation
// Global: begining and ending times read in to track onset of global events 
// (about 50 time units (100 times steps) ahead of the spike itself, determined by threshold))
double beg_times[50000];
double end_times[50000];
char* global_events_fn = "all_extremities_seed1000_dc0.10.txt"; // file to read global times from
...
*/
{
  ...
  // Turn off soma (cytool and CR) through its currents when Ca in the cytosol is above a certain threshold
   ...
   printf("before first if: end, t= %f, %f\n", end_times[next_time], t);
   if (t > end_times[next_time]) {
   	  printf("*****  update next_time *****\n");
   	  next_time += 1;
   }

   printf("beg, end= %f, %f,   t= %f\n", beg_times[next_time], end_times[next_time], t);
   if (t >= beg_times[next_time] && t <= end_times[next_time]) {
      f_factor = 0.0;
   }
...
int readData()
// return time_sz: number of elements read + 1
{
	FILE* fd = fopen(global_events_fn, "r");
	printf("fn= %s\n", global_events_fn);
	printf("fd= %ld\n", fd);
	//printf("fd= %ld\n", fd);
	double f1, f2;
	...
	for (i=0; i < 5000; i++) {
	    printf("i= %d\n", i);
		if (fscanf(fd, "%lf %lf\n", &f1, &f2) == 2) {
			beg_times[i] = f1 * 0.5;  // convert index to times (dt = 0.5; Must automate this in case time step changes). HARDCODED FOR NOW. GE. 
			end_times[i] = f2 * 0.5;  // convert index to times (dt = 0.5; Must automate this in case time step changes). HARDCODED FOR NOW. GE. 
			printf("times[%d] = %f, %f\n", i, beg_times[i], end_times[i]);
		} else {
			break;
		}
	}
	beg_times[i] = 5000000000; // hardcoding is dangerous
	end_times[i] = 5000000000; // hardcoding is dangerous
	i++;
	printf("beg_times[%d] = %lf, end_times[%d]= %lf\n", i-1, beg_times[i-1], i-1, end_times[i-1]);
	printf("****  return: %d\n", i); 
	return i;
}
