...

{
  ...
  sigma_heterogeneous[0] = 0.05;
  sigma_heterogeneous[1] = 0.10;
  sigma_heterogeneous[2] = 0.15;
  sigma_heterogeneous[3] = 0.20;
  sigma_heterogeneous[4] = 0.25;

  printf("\n");
  for (int i=0; i < NPROC; i++) {
     printf("process %d, sigma_heterogneous[%d]: %5.3f\n", i, i, sigma_heterogeneous[i]);
  }
  printf("\n");
...
*/
{
  ...
  for(ii = 0;ii<NPROC; ii++) 
  {
	  ...
	  dr[ii] = r4_normal_ab ( mu, sigma_heterogeneous[ii], &seed );
...
#ifdef VD
  yp[NEQN-2] = ( (r -c4 * f(y[NEQN-2],y[NEQN-1]) - y[NEQN-2]) * Vd_surf + cytosol_diff) / (Vd*tau); //soma cytosol
